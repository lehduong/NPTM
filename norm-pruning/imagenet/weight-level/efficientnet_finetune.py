...
"""
from utils import accuracy, get_conv_zero_param, PruningApexScaler
import wandb
import argparse
import time
import yaml
...
parser.add_argument('--finetune', default='', type=str, metavar='PATH',
                    help='Finetune pruned model (default: none)')
...
parser.add_argument('-b', '--batch-size', type=int, default=384, metavar='N',
...
parser.add_argument('--lr', type=float, default=0.048, metavar='LR',
...
parser.add_argument('--decay-rate', '--dr', type=float, default=0.97, metavar='RATE',
...
parser.add_argument('--drop-connect', type=float, default=0.2, metavar='PCT',
...
parser.add_argument(
    ...
    type=float,
    default=320,
    help='div factor of OneCycle Learning rate Schedule (default: 320)')
...
def main():
    ...
    if args.finetune:
        if os.path.isfile(args.finetune):
            ...
            raise ValueError(
                "No checkpoint found at '{}'".format(args.finetune))
    ...
    if args.use_onecycle:
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, div_factor=args.div_factor,
                                                           epochs=num_epochs, steps_per_epoch=len(loader_train), pct_start=args.pct_start,
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(output_base, 'finetune')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            eval_metrics = validate(
                model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, optimizer=optimizer)

            ret = {'top1': eval_metrics["top1"], 'top5': eval_metrics["top5"],
                   'best_top1': best_metric, 'lr': cur_lr}
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                ret['ema_top1'] = eval_metrics['top1']
                ret['ema_top5'] = eval_metrics['top5']
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if loss_scaler is not None:
            loss_scaler(loss, optimizer, model)
        else:
            loss.backward()
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', optimizer=None):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
