...
class pytorch_pruning(object):
    def __init__(self, parameters, pruning_settings=dict(), log_folder=None):
        ...
        initialize_parameter(self, pruning_settings,
                             'flops_regularization', 0.0)
        ...
        self.prune_network_accomulate = {
            "by_layer": list(), "averaged": list(), "averaged_cpu": list()}
        ...
        if self.method == 40 or self.method == 50 or self.method == 25:
            self.oracle_dict = {"layer_pruning": -1, "initial_loss": 0.0,
                                "loss_list": list(), "neuron": list(), "iterations": 0}
        ...
        self.data_logger["time_stamp"] = time.strftime(
            "%Y_%m_%d_%H_%M_%S", time.localtime())

        if hasattr(self, 'seed'):
            self.data_logger["seed"] = self.seed

        self.data_logger["filename"] = "%s/data_logger_seed_%d_%s.p" % (
            log_folder, self.data_logger["seed"], self.data_logger["time_stamp"])
        if self.method == 50:
            self.data_logger["filename"] = "%s/data_logger_seed_%d_neuron_%d_%s.p" % (
                log_folder, self.starting_neuron, self.data_logger["seed"], self.data_logger["time_stamp"])
        self.log_folder = log_folder
    ...
    def add_criteria(self):
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            if len(self.pruning_mask_from) > 0:
                ...
                self.method = -1
                criteria_for_layer = torch.from_numpy(
                    self.loaded_mask_criteria[layer]).type(torch.FloatTensor).cuda()
            ...
            if self.method == 0:
                ...
                # random pruning
                criteria_for_layer = np.random.uniform(
                    low=0, high=5, size=(nunits,))
            elif self.method == 2:
                # min weight
                criteria_for_layer = self.parameters[layer].pow(
                    2).view(nunits, -1).sum(dim=1).data
            elif self.method == 3:
                # weight_abs
                criteria_for_layer = self.parameters[layer].abs().view(
                    nunits, -1).sum(dim=1).data
            elif self.method == 6:
                # ICLR2017 Taylor on output of the layer
                if 1:
                    criteria_for_layer = self.parameters[layer].full_grad_iclr2017
                    criteria_for_layer = criteria_for_layer / \
                        (np.linalg.norm(criteria_for_layer) + eps)
            elif self.method == 10:
                # diagonal of Hessian
                ...
                criteria_for_layer = (self.parameters[layer].full_grad.t()).data.pow(
                    2).view(nunits, -1).sum(dim=1)

            elif self.method == 30:
                # batch normalization based pruning
                # by scale (weight) of the batchnorm
                criteria_for_layer = (self.parameters[layer]).data.abs().view(
                    nunits, -1).sum(dim=1)
                ...
                criteria_for_layer = np.asarray(
                    self.oracle_dict["loss_list"][layer]).copy()
    ...
    def group_criteria(list_criteria_per_layer, group_size=1):
        ...
        for layer in list_criteria_per_layer:
            ...
            for group_id in range(int(np.ceil(len(layer)/group_size))):
                current_group = slice(
                    group_id*group_size, min((group_id+1)*group_size, len(layer)))
    ...
    def compute_saliency(self):
        ...
        if validation_error > self.pruning_threshold:
            ...
            print("skipping pruning", validation_error, "(%f)" %
                  validation_error_long, self.pruning_threshold)
            if self.method != 4:
                self.res_pruning = -1
                return -1
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            if self.iterations_done > 0:
                ...
                contribution = self.prune_network_accomulate["by_layer"][layer] / \
                ...
                if self.pruning_iterations_done == 0 or not self.use_momentum or (self.method in [4, 40, 50]):
                    ...
                    self.prune_network_accomulate["averaged"][layer] = self.momentum_coeff * \
                        self.prune_network_accomulate["averaged"][layer]+(
                ...
                if self.l2_normalization_per_layer:
                    ...
                    current_layer = current_layer / \
                        (np.linalg.norm(current_layer) + eps)
        ...
        if not self.pruning_silent:
            ...
            pickle.dump(store_criteria, open(self.folder_to_write_debug +
                                             "criteria_%04d.pickle" % self.pruning_iterations_done, "wb"))
            if self.pruning_iterations_done == 0:
                pickle.dump(store_criteria, open(self.log_folder +
                                                 "criteria_%d.pickle" % self.method, "wb"))
            pickle.dump(store_criteria, open(self.log_folder +
                                             "criteria_%d_final.pickle" % self.method, "wb"))

        if not self.fixed_criteria:
            self.iterations_done = 0

        # create groups per layer
        groups = self.group_criteria(
            self.prune_network_criteria, group_size=self.group_size)
        ...
        all_criteria = np.asarray(
            [group[1] for layer in groups for group in layer]).reshape(-1)

        prune_neurons_now = (self.pruned_neurons +
                             self.prune_per_iteration)//self.group_size - 1
        if self.prune_neurons_max != -1:
            prune_neurons_now = min(len(
                all_criteria)-1, min(prune_neurons_now, self.prune_neurons_max//self.group_size - 1))
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            write_to_debug("pruned_perc:", [np.nonzero(
                1.0-self.pruning_gates[layer])[0].size, len(self.parameters[layer])])
        ...
        try:
            ...
            self.median_criteria_value = np.median(
                all_criteria[all_criteria > 0.0])
        except:
    ...
    def set_weights_oracle_pruning(self):
        ...
        for layer, if_prune in enumerate(self.prune_layers_oracle):
            ...
            if self.method == 40:
                self.parameters[layer].data = deepcopy(
                    torch.from_numpy(self.stored_weights).cuda())
        ...
        for layer_indx, parameter in enumerate(self.parameters):
            ...
            if self.prune_layers[layer_indx]:
                ...
                for parameter_loc in range(len(parameter)):
                    ...
                    grad_params2 = torch.autograd.grad(
                        grad_params[0][parameter_loc], parameter, create_graph=True)
            ...
            self.temp_hessian.append(
                torch.FloatTensor(hessian.cpu().numpy()).cuda())
    ...
    def run_full_oracle(self, model, data, target, criterion, initial_loss):
        ...
        if self.method == 40:
            ...
            for layer_index, layer_parameters in enumerate(self.parameters):
                ...
                self.stored_weights = deepcopy(
                    self.parameters[layer_index].data.cpu().numpy())
                ...
                for neurion_id, neuron in enumerate(layer_parameters):
                    ...
                    if len(self.oracle_dict["loss_list"][layer_index]) == 0:
                        self.oracle_dict["loss_list"][layer_index] = [
                            oracle_value, ]
                    elif len(self.oracle_dict["loss_list"][layer_index]) < neurion_id+1:
                        self.oracle_dict["loss_list"][layer_index].append(
                            oracle_value)
                    else:
            ...
            if self.pruning_iterations_done == 0:
                # store weights again
                self.stored_weights = deepcopy(
                    self.parameters[self.fixed_layer].data.cpu().numpy())
    ...
    def report_loss_neuron(self, training_loss, training_acc, train_writer=None, neurons_left=0):
        ...
        if train_writer is not None:
            train_writer.add_scalar(
                'loss_neuron', training_loss, self.all_neuron_units-self.neuron_units)

        self.data_logger["pruning_neurons"].append(
            self.all_neuron_units-self.neuron_units)
    ...
    def load_mask(self):
        ...
        if not len(self.pruning_mask_from) > 0:
            ...
            if not os.path.isfile(self.pruning_mask_from):
                print("pruning_engine.load_mask(): file doesn't exist",
                      self.pruning_mask_from)
                print("pruning_engine.load_mask(): check it, exit,",
                      self.pruning_mask_from)
                exit()
            ...
            print("pruning_engine.load_mask(): loaded criteria from",
                  self.pruning_mask_from)
    ...
    def report_to_tensorboard(self, train_writer, processed_batches):
        ...
        train_writer.add_scalar(
            'Neurons_left', self.neuron_units, processed_batches)
        train_writer.add_scalar(
            'Criteria_min', self.min_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar(
            'Criteria_max', self.max_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar(
            'Criteria_median', self.median_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar(
            'Gradient_norm_before', gradient_norm_final_before, self.pruning_iterations_done)
        train_writer.add_scalar(
            'Pruning_threshold', self.threshold_now, self.pruning_iterations_done)
    ...
    def do_step(self, loss=None, optimizer=None, neurons_left=0, training_acc=0.0):
        ...
        if ((niter-1) % self.frequency == 0) and (niter != 0) and (self.res_pruning == 1):
            self.report_loss_neuron(self.util_training_loss, training_acc=self.util_training_acc,
                                    train_writer=self.train_writer, neurons_left=neurons_left)
        ...
        if niter % self.frequency == 0 and niter != 0:
            ...
            if self.res_pruning == 1:
                print("Pruning: Units", self.neuron_units, "/", self.all_neuron_units, "loss", training_loss, "Zeroed",
                      self.pruned_neurons, "criteria min:{}/max:{:2.7f}".format(self.min_criteria_value, self.max_criteria_value))
        ...
        for layer in range(len(self.pruning_gates)):
            ...
            for unit in range(len(self.pruning_gates[layer])):
                if not self.pruning_gates[layer][unit]:
                    continue
                if 'momentum_buffer' in optimizer.state[self.parameters[layer]].keys():
                    optimizer.state[self.parameters[layer]
                                    ]['momentum_buffer'][unit] *= 0.0
    ...
    def update_flops(self, stats=None):
        ...
        for pruning_param in self.gates_to_params:
            if isinstance(pruning_param, list):
                ...
                self.per_layer_flops.append(
                    sum([stats['flops'][a] for a in pruning_param]))
            else:
    ...
    def apply_flops_regularization(self, groups, mu=0.1):
        ...
        for layer_id, layer in enumerate(groups):
            for group in layer:
                ...
                group[1] = group[1] - mu * \
                    (self.per_layer_flops[layer_id]*total_neurons)
...
def prepare_pruning_list(pruning_settings, model, model_name, pruning_mask_from='', name=''):
                ...
                if (pruning_method != 23) and (pruning_method != 6):
                    ...
                    def just_hook(self, grad_input, grad_output):
                        ...
                        if len(grad_output[0].shape) == 4:
                            self.weight.full_grad = (
                                grad_output[0] * self.output).sum(-1).sum(-1)
                        else:
                            self.weight.full_grad = (
                                grad_output[0] * self.output)
                    ...
                    if pruning_method == 6:
                        ...
                        def just_hook(self, grad_input, grad_output):
                            if len(grad_output[0].shape) == 4:
                                ...
                                self.weight.full_grad_iclr2017 = (
                                    grad_output[0] * self.output).abs().mean(0)
    ...
    if model_name == "resnet20":
        ...
        pruning_parameters_list = [d for di, d in enumerate(
            pruning_parameters_list) if (di % 2 == 1 and di > 0)]
